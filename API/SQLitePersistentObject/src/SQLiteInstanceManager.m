...
- (void)executeUpdateSQL:(NSString *) updateSQL
{
	...
	if (sqlite3_exec([self database],[updateSQL UTF8String] , NULL, NULL, &errorMsg) != SQLITE_OK) {
		//NSString *errorMessage = [NSString stringWithFormat:@"Failed to execute SQL '%@' with message '%s'.", updateSQL, errorMsg];//alan comment
        
	//	NSAssert(0, errorMessage); //alan comment
