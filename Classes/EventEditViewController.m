...
-(void) initViewController
{
	...
	[((UIButton *) topView.topRightButton) setTitle:NSLocalizedString(@"Remind",@"[invite]") forState:UIControlStateNormal];
...
- (void)showData 
{
	...
	if(inviteObj.EventTitle)
	{
		titleTextField.text=inviteObj.EventTitle;
	}
	
	if(inviteObj.dateString)
	{
		whenTextField.text=[ACUtility dateStringToDateFormatString:inviteObj.dateString];
	}
	
	NSLog(@"%@",appDelegate.temp.inviteObj.contactDict);
	[self callBackChangeContactListFunction];
...
- (void)gotoContactListAction 
{
	...
	EditContactListViewController *VC=[[EditContactListViewController alloc] initWithNibName:@"EditContactListViewController" bundle:nil];
...
- (void)submitAction 
{
	...
	{
		if(![[array objectAtIndex:i] objectForKey:@"alreadyIn"])
		{
			if(contact_list==nil)
			{
	...
	[self editEventConnection:network 
						   ID:inviteObj.eid
				 restaurantID:rid
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
	...
	{
		SearchRestaurantViewController *VC=[[SearchRestaurantViewController alloc] initWithNibName:@"SearchRestaurantViewController" bundle:nil];
		[self.navigationController pushViewController:VC animated:YES];
		[VC release];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if(![dict objectForKey:@"alreadyIn"])
	{
	...
	{
		NSString *firstname=[dict objectForKey:@"display_name"];
		...
		cell.label1.text=[NSString stringWithFormat:@"%@",firstname];
...
{	
	...
	if( [[request.additionDataDictionary objectForKey:@"id"] isEqualToString:@"editEvent"])
	{
		appDelegate.temp.inviteObj=nil;
