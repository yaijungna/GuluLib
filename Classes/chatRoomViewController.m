...
-(void)enterChatRoom
{
   // NSLog(@"%@,%@",chatID,[chatDictionary objectForKey:@"participant_uuid"]);
     ACLog(@"chatID =%@,participant_uuid=%@",chatID,[chatDictionary objectForKey:@"participant_uuid"]);
	socket=chat.socket;
//	[chat subscribe:chatID participants:[chatDictionary objectForKey:@"participant_uuid"]];
    [chat join_chat:chatID];
...
{
    //NSLog(@"%@",user_ID);
     ACLog(@"user_ID=%@",user_ID);
	socket=chat.socket;
//    [chat subscribe:@"" participants:appDelegate.userMe.UUID];
    [chat create_chat:user_ID];
    createWithUserUUID=user_ID;
}
