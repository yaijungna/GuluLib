...
@implementation dishProfileViewController

@synthesize dish;
@synthesize dishPhotoArray;
@synthesize dishPhotoRequest;

-(void)initViewController
{
	restaurantView=[[dishProfileView alloc] initWithFrame:CGRectMake(0, 53, 320, 120)];
	...
	table =[[GuluTableView alloc] initWithFrame:CGRectMake(0, 170, 320, 240)   pullToRefresh:YES];
...
- (void)showData 
{
	restaurantView.titleLabel.text=[NSString stringWithFormat:@"%@ @ %@",dish.name, dish.restaurant.name];	
	
	restaurantView.follwersNumberLabel.text=@"0";
	restaurantView.reviewsNumberLabel.text=@"0";
    
    [restaurantView.imageView setImageWithURL:[NSURL URLWithString:dish.photo.image_medium]];
...
- (void)dealloc {
    [dish release];
    [spinView release];
    [dishPhotoRequest release];
	[dishPhotoArray release];
...
{
	static NSString *cellIdentifier = @"GuluTableViewCell_3Images";
	GuluTableViewCell_3Images *cell=[GuluTableViewCell cellForIdentifierOfTable:cellIdentifier table:theTableView];
    ...
    if(index1<number)
    {
        GuluPhotoModel *photo=[tableArray objectAtIndex:index1];
        [cell.btn1 setBackgroundImageWithURL:[NSURL URLWithString:photo.image_medium] forState:UIControlStateNormal];
    }
    if(index2<number)
    {
         GuluPhotoModel *photo=[tableArray objectAtIndex:index2];
        [cell.btn2 setBackgroundImageWithURL:[NSURL URLWithString:photo.image_medium] forState:UIControlStateNormal];
    }
    if(index3<number)
    {
         GuluPhotoModel *photo=[tableArray objectAtIndex:index3];
        [cell.btn3 setBackgroundImageWithURL:[NSURL URLWithString:photo.image_medium] forState:UIControlStateNormal];
    }
...
{	
    ...
    self.dishPhotoRequest=[[GuluAPIAccessManager sharedManager] photosOfDish:self dishID:dish.ID];
}
...
- (void)GuluAPIAccessManagerSuccessed:(GuluHttpRequest*)httpRequest info:(id)info
{
    ...
    if( httpRequest == dishPhotoRequest)
	{
		self.dishPhotoArray=[[[NSMutableArray alloc] initWithArray:info] autorelease];
		tableArray=dishPhotoArray;
		[table reloadData];
	}
}
