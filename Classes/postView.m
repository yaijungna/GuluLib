...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		imageView.image=[UIImage imageNamed:@"post-splash-graphic-1.png"];
		...
		mainLabel.text=LANDING_POST_TITLE_STRING;
		[self addSubview:mainLabel];
		
		label1=[[UILabel alloc] initWithFrame:CGRectMake(190, 50, 90, 40)];
		label1.backgroundColor=[UIColor clearColor];
		label1.textAlignment=UITextAlignmentCenter;
		label1.font=[UIFont fontWithName:FONT_BOLD size:20];
		label1.textColor=TEXT_COLOR;
		label1.numberOfLines=1;
		label1.text=LANDING_POST_PHOTO_STRING;
